...
class ModerationEngine:
    ...
    async def _persist_cluster(self, own_cluster: ClusterInfo | None) -> int | None:
        ...
        # upsert_cluster_by_members_ex() находит уже открытый активный
        # кластер по общим участникам и обновляет ЕГО, возвращая тот же id,
        # что был выдан при первом обнаружении. Без store (SHADOW-тесты,
        # replay.py)
